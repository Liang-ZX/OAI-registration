...
def prepare_reference(all_image_data):
    ...
    if image_data["registration_type"] == "newsubject":
        ...
        # instanciate vessel class and prepare reference
        image_data["current_anatomy"] = image_data["vessel"]
        vessel = elastix_transformix.vessel()
        vessel.prepare_reference (image_data)
...
#    print ("-> Registering " + image_data["moving_root"])
    ...
    if image_data["registration_type"] == "newsubject" or image_data["registration_type"] == "longitudinal":
        # instantiate vessel class and provide vessel to segment
        image_data["current_anatomy"] = image_data["vessel"]
        vessel = elastix_transformix.vessel()

        # register
        vessel.spline(image_data)
    else:
        print ("-> Step skipped")

def register_vessel_to_reference(all_image_data, n_of_processes):

    start_time = time.time()
    pool = multiprocessing.Pool(processes=n_of_processes)
    pool.map(register_vessel_to_reference_s, all_image_data)
    print ("-> Registration completed")
    print ("-> The total time was %.2f seconds (about %d min)" % ((time.time() - start_time), (time.time() - start_time)/60))
...
#    print ("-> Inverting transformation of " + image_data["moving_root"])
    ...
    if image_data["registration_type"] == "newsubject" or image_data["registration_type"] == "longitudinal":
        ...
        # invert transformations
        vessel.i_spline (image_data)

    elif image_data["registration_type"] == "multimodal":
        print ("-> Step skipped")

def invert_vessel_transformations(all_image_data, n_of_processes):

    start_time = time.time()
    pool = multiprocessing.Pool(processes=n_of_processes)
    pool.map(invert_vessel_transformations_s, all_image_data)
    print ("-> Inversion completed")
    print ("-> The total time was %.2f seconds (about %d min)" % ((time.time() - start_time), (time.time() - start_time)/60))
...
#    print ("-> Warping vessel mask of " + image_data["moving_root"])
    ...
    if image_data["registration_type"] == "newsubject":
        ...
        # modify transformations for mask warping
        vessel.modify_transformation(image_data,"spline")

        # warp mask
        vessel.t_spline    (image_data)
        vessel.t_similarity(image_data)
        vessel.t_rigid     (image_data)
...
def warp_vessel_mask(all_image_data, n_of_processes):
    ...
    start_time = time.time()
    pool = multiprocessing.Pool(processes=n_of_processes)
    pool.map(warp_vessel_mask_s, all_image_data)
    print ("-> Warping completed")
    print ("-> The total time was %.2f seconds (about %d min)" % ((time.time() - start_time), (time.time() - start_time)/60))
