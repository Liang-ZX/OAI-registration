...
def read_dicom_stack_s(image_data):
    ...
    # save image to temp
    sitk.WriteImage(sitk.Cast(img, sitk.sitkFloat32), image_data["temp_file_name"])
...
def axial_to_sagittal_s(image_data):
    # read the image
    img = sitk.ReadImage(image_data["temp_file_name"])

    # change orientation
    img = sitkf.axial_to_sagittal(img)

    # save image to temp
    sitk.WriteImage(img, image_data["temp_file_name"])
    
    
def axial_to_sagittal(all_image_data, n_of_processes):

    start_time = time.time()
    pool = multiprocessing.Pool(processes=n_of_processes)
    pool.map(axial_to_sagittal_s, all_image_data)
